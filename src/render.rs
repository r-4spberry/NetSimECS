...
use macroquad::prelude::{clear_background, draw_line, DARKGRAY};
...
fn draw_devices(world: &World) {
    for (device, (label, trans, ports, color)) in
...
fn draw_links(world: &World) {
    for (link, (connects, color)) in world.query::<(&ConnectsPorts, &Color)>().iter() {
        let w_a =
        ...
        for (packet, trans) in world.query::<(&Transit)>().iter() {
